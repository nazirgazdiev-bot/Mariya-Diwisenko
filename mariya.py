...
- Все блюда разложены в боте по 6 категориям: Первые блюда, Блюда из мяса,
  Блюда из рыбы, Салаты-закуски-завтраки-несладкая выпечка, Гарниры и каши,
  Десерты и сладкая выпечка.
- В рационе ВСЕГДА показывай клиенту, где найти блюдо в меню. Формат:
  «Как найти: Категория → Название».
...
Как найти: Салаты, закуски, завтраки, несладкая выпечка → Овсяноблин

ОБЕД
Лазанья — 1 порция → 422 ккал, Б 41 / Ж 9 / У 42
(КБЖУ указан на порцию, беру как есть)
Как найти: Блюда из мяса → Лазанья
...
def build_recipe_book_text(menu_data: dict) -> str:
    ...
    Структура: КАТЕГОРИЯ → РЕЦЕПТЫ (без подкатегорий).
    """
    out = [
        f"СБОРНИК РЕЦЕПТОВ МАРИИ ДИВИСЕНКО (всего {menu_data['total']} рецептов).",
        "",
        "Меню в боте устроено как: КАТЕГОРИЯ → РЕЦЕПТ.",
    ...
    for cat, recipe_ids in menu_data["menu"].items():
        if not recipe_ids:
            continue
        out.append("")
        out.append(f"### КАТЕГОРИЯ: {cat} ({len(recipe_ids)} рецептов)")

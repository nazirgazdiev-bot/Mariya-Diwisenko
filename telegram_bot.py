...
START_TEXT = (
    ...
    "Что я умею:\n"
    "— Составлять рационы на день, неделю, месяц именно под твои цели по КБЖУ\n"
    ...
    "Чтобы я могла составить тебе рацион — расскажи о себе:\n"
    "1. Какая цель — похудение, поддержание или набор массы?\n"
    "2. Сколько калорий в день тебе нужно?\n"
    "3. Какие БЖУ (белки/жиры/углеводы) в граммах?\n"
    "4. Есть ли аллергии или продукты, которые ты не ешь?\n"
    "5. Что любишь — может, сладкое или острое?\n\n"
    "Можешь рассказать всё сразу одним сообщением, или по частям — "
    "я задам уточняющие вопросы.\n\n"
    "Пример: «Хочу похудеть, 1500 ккал, БЖУ 120/50/130, "
    "не ем рыбу и грибы, обожаю сладкое. Составь рацион на день».\n\n"
    "Команды:\n"
    "/help — это сообщение\n"
...
async def main():
    ...
    if not recipes_path.exists():
        raise RuntimeError(f"recipes.json не найден: {recipes_path}")
    recipes_data = json.loads(recipes_path.read_text(encoding="utf-8"))
    ...
    me = await bot.me()
    log.info(
        "МарИИя-Telegram @%s готова (model=%s, recipes=%d, db=%s)",
        me.username, MODEL, recipes_data["total"], DB_PATH,
    )

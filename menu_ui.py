...
def _format_instructions(raw: str) -> str:
    """
    Чистит сырой текст инструкций из PDF и разбивает на абзацы.
    PDF-парсинг даёт:
      - Слово "ПРИГОТОВЛЕНИЕ" внутри текста (служебный заголовок)
      - Склейки переносов: "приготов- ления" → "приготовления"
      - Склейки прописных: "минутПРИГОТОВЛЕНИЕ"
      - Прилипшие заголовки: "БЛЮДА ИЗ МЯСА И РЫБЫ 64"
    """
    if not raw:
        return ""
    text = raw

    # 1) Убрать склейки переносов: "слово-  слово" → "словослово"
    text = re.sub(r"([а-яё])-\s+([а-яё])", r"\1\2", text)

    # 2) Вставить разрыв перед слипшимся "ПРИГОТОВЛЕНИЕ"
    text = re.sub(r"(\S)(ПРИГОТОВЛЕНИЕ)", r"\1. \2", text)

    # 3) Удалить служебные заголовки целиком
    text = re.sub(r"\bПРИГОТОВЛЕНИЕ\b", "", text)
    text = re.sub(r"\bИНГРЕДИЕНТЫ\b", "", text)
    # Шапки страниц вида "БЛЮДА ИЗ МЯСА И РЫБЫ 64"
    text = re.sub(
        r"\b(?:ПЕРВЫЕ БЛЮДА|БЛЮДА ИЗ МЯСА(?:\s+И РЫБЫ)?|БЛЮДА ИЗ РЫБЫ|"
        r"САЛАТЫ[А-ЯЁ ,]*ВЫПЕЧКА|ГАРНИРЫ И КАШИ|ДЕСЕРТЫ И СЛАДКАЯ ВЫПЕЧКА)\s+\d+",
        "",
        text,
    )
    ...
    # 5) Разбить на предложения и собрать абзацами
    sentences = re.split(r"(?<=[.!?])\s+(?=[А-ЯЁA-Z])", text)
    sentences = [s.strip() for s in sentences if s.strip()]
    # Один абзац = одно предложение (короткие предложения склеим)
    paragraphs = []
    buf = ""
    for s in sentences:
        # Очень короткое предложение — добавляем к буферу
...
def text_recipe(recipe_id: str) -> str:
    ...
    instr = _format_instructions(r.get("instructions") or "")
    if instr:
